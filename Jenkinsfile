pipeline {
          ...
          environment {
            ...
            steps {
              withCredentials([string(credentialsId: 'sonarqube-token', variable: 'SONAR_TOKEN')]) {
                      ...
                      -Dsonar.host.url=http://3.92.193.122:9000 \
    ...
    stage('4. Docker Image Build') {
      steps {
        sh "aws ecr-public get-login-password --region us-east-1 | sudo docker login --username AWS --password-stdin public.ecr.aws/x3j1z5l3"
        sh "sudo docker build -t addressbook ."
        sh "sudo docker tag addressbook:latest public.ecr.aws/x3j1z5l3/addressbook:${params.ecr_tag}"
        sh "sudo docker push public.ecr.aws/x3j1z5l3/addressbook:${params.ecr_tag}"
      }
    }
    ...
    stage('7. Email Notification') {
      steps {
        mail bcc: 'akingab@gmail.com', body: '''Build is Over. Check the application using the URL below:
         https://abook.dominionsystem.com/addressbook-1.0
         Let me know if the changes look okay.
         Thanks,
         Dominion System Technologies,
         +1 (313) 413-1477''', 
         subject: 'Application was Successfully Deployed!!', to: 'akingab@gmail.com'
      }
    }
  }
}
